 ...
 * C3-PRO
 * <p>
 * Created by manny Weber on 07/28/2016.
 ...
 */
public class QuestionnaireItemAsStep {
    /**
     ...
     */
    public static List<Step> getQuestionnaireItemsAsSteps(List<Questionnaire.QuestionnaireItemComponent> items) {
        List<Step> steps = new ArrayList<>();

        for (Questionnaire.QuestionnaireItemComponent item : items) {

            if (item.getType() == Questionnaire.QuestionnaireItemType.GROUP) {
                List<Step> newSteps = getQuestionnaireItemsAsSteps(item.getItem());
                if (item.hasEnableWhen()) {
                    List<ResultRequirement> reqs = getRequirementsForQuestionnaireItem(item);
                ...
                Step newStep = getQuestionnaireItemAsStep(item);
     ...
     * {@link #getQuestionnaireItemsAsSteps(List)}.) If the Item has {@link org.hl7.fhir.dstu3.model.Questionnaire.QuestionnaireItemEnableWhenComponent}
     ...
     */
    private static Step getQuestionnaireItemAsStep(Questionnaire.QuestionnaireItemComponent item) {
        ...
        if (item.getType() == Questionnaire.QuestionnaireItemType.DISPLAY) {
            ...
            AnswerFormat fmt = getAnswerFormatForItem(item);
     ...
     */
    private static AnswerFormat getAnswerFormatForItem(Questionnaire.QuestionnaireItemComponent item) {
     ...
     */
    private static String getQuestionInstruction(Questionnaire.QuestionnaireItemComponent item) {
     ...
     */
    private static String getQuestionHelpText(Questionnaire.QuestionnaireItemComponent item) {
     ...
     */
    private static String getNumericAnswerUnit(Questionnaire.QuestionnaireItemComponent item) {
     ...
     */
    private static Extension getDefaultAnswer(Questionnaire.QuestionnaireItemComponent item) {
    ...
    private static String getAlternativeTextForItem(Questionnaire.QuestionnaireItemComponent item) {
        String instr = getQuestionInstruction(item);
     ...
     */
    private static List<ResultRequirement> getRequirementsForQuestionnaireItem(Questionnaire.QuestionnaireItemComponent item) {
